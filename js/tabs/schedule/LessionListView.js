...
class LessionListView extends React.Component {
  props: Props;
  state: State;
  ...
  constructor(props: Props) {
    super(props);
    this.state = {
      todaySessions: groupSessions(FilterSessions.byDay(props.sessions, props.lession)),
    };
  ...
  componentWillReceiveProps(nextProps: Props) {
    if (nextProps.sessions !== this.props.sessions ||
        nextProps.lession !== this.props.lession) {
      this.setState({
        todaySessions: groupSessions(FilterSessions.byDay(nextProps.sessions, nextProps.lession)),
      });
    }
  }
  ...
  renderRow(session: Session, lession: number) {
    return (
      <F8SessionCell
        onPress={() => this.openSession(session, lession)}
        session={session}
      />
    );
  }
  ...
  renderEmptyList(): ?ReactElement {
    ...
    return renderEmptyList(this.props.session);
  }

  openSession(session: Session, lession: number) {
...
module.exports = LessionListView;
